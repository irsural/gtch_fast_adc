  ...
  mp_tim(reinterpret_cast<tim_regs_t*>(a_timer_address))
{
  ...
  mp_tim->TIM_ARR = irs::cpu_traits_t::frequency()/freq_50_hz;
...
void interrupt_generator_t::interrupt_generator_t::add_event(
  irs::event_t* ap_event)
{
  switch (reinterpret_cast<size_t>(mp_tim)) {
    case IRS_TIM1_PWM1_BASE: {
      irs::interrupt_array()->int_gen_events(irs::arm::tim1_up_tim10_int)->
        push_back(ap_event);
      NVIC_SetPriority (TIM1_UP_TIM10_IRQn, 0);
    } break;
    case IRS_TIM8_PWM2_BASE: {
      irs::interrupt_array()->int_gen_events(irs::arm::tim8_up_tim13_int)->
        push_back(ap_event);
...
void interrupt_generator_t::interrupt_generator_t::erase_event(
  irs::event_t* ap_event)
{
  switch (reinterpret_cast<size_t>(mp_tim)) {
    case IRS_TIM1_PWM1_BASE: {
